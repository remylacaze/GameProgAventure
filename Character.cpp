...
Character::Character(string name, Type classType,
...
std::string Character::getStringClassFromEnumType(Type classType)
{
	switch (classType)
	{
	case Type::Warrior:
...
//Methods
//Character* Character::makeCharacter(int choice, string name, Type classType,
//	int distanceTravelled,
//	int gold, int level,
//	int exp, int strength, int vitality,
//	int dexterity, int intelligence,
...
//{
//	if (choice == 1)
//		return new Warrior(name, classType,
//			distanceTravelled,
//			gold, level,
//			exp, strength, vitality,
...
string Character::getInvAsStringSave()
{
	...
	for (size_t i = 0; i < this->inventory.size(); i++)
	{
		if (this->inventory[i].getItemType() == Item::Type::WEAPON)
	...
	for (size_t i = 0; i < this->inventory.size(); i++)
	{
		if (this->inventory[i].getItemType() == Item::Type::ARMOR)
			inv += this->inventory[i].toStringSave();
	}
