...
{
public:
	enum Type { Warrior = 0, Mage = 1, Archer = 2, Explorer = 3 };
	static std::string getStringClassFromEnumType(Type classType);

public:
	Character();
	Character(string name, Type classType,
...
protected:
	// Singleton
	//static Character* instance_; // Uncomment for singleton

	Type classType = Type::Warrior;
	...
	// Operator
	//Character& operator= (const Character&) {} // Uncomment for singleton

public:
	//static Character* getInstance();
	static Character* makeCharacter(int choice, string name, Type classType,
		int distanceTravelled,
		int gold, int level,
		int exp, int strength, int vitality,
		int dexterity, int intelligence,
