...
DynamicArray<std::string> Weapon::names;
...
Weapon::Weapon(int level, int rarity)
	:Item(Item::Type::WEAPON, level, rarity)
{
	if (level == 0) { level = 1; } // Avoid division by zero
...
Weapon::Weapon(int damageMin, int damageMax, std::string name, int level, int buyValue, int sellValue, int rarity)
	: Item(Item::Type::WEAPON, name, level, buyValue, sellValue, rarity)
{
